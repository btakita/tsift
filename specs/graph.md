...
`graph-db` output flags are global to the command group, so `--json` is accepted
both before and after every graph-db subcommand. Singular lookups (`node`, `edge`,
`incident`, `kind`, and `neighborhood`) fail with a non-zero exit and a diagnostic
when their requested target or kind does not exist. A missing or stale projection
diagnostic includes `tsift graph-db --json refresh` as the repair command.
`tsift graph-db refresh --rebuild --json` forces the same transactional projection
replacement even when the stored source watermark matches, providing a supported
repair path for stale semantic rows without deleting `graph.db` by hand.
Scoped SQLite reads also check that `.tsift/indexes/<scope>/graph.db` exists before
opening it and fail with the exact scoped `graph-db ... refresh --json` command;
they never expose the raw SQLite open error. Human node output includes
`location: <path>[:<line>]` whenever the node has source location properties.

When graph projection is requested at a workspace root without an explicit scope,
the code tier federates the root and every discovered workspace search index.
Projected `file`, `symbol`, and `ast_span` nodes carry their source scope (`<root>`
or the workspace scope id), and freshness covers every federated index. Context-pack
resume commands do not repeat guessed `graph-db evidence` targets; only targets
resolved against the live projection are emitted as graph follow-up commands.
An explicitly scoped projection also tags `route` and semantic entity nodes with
...
All strategies add the same graph-native freshness and memory signals before
pruning. The freshness signal reads `GraphFreshness.observed_at_unix`, node
property `observed_at_unix`, or node property `max_observed_at_unix`, then applies
a half-life bucket bonus (`observed_at_weight`, default 24; default half-life one
week). The memory signal boosts `memory_event`, `memory_session`, `tsift-memory`
source/semantic nodes, and authored `finding` / `decision` / `note` nodes; authored
nodes also get a clamped confidence bonus. `memory_projection` metadata nodes are
excluded from the boost. This lets memory and code nodes use one
`GraphStore::ranked_neighborhood` pruning path instead of caller-specific
post-ranking.
...
The `RankedNeighborhoodOptions` struct carries `depth`, `max_nodes`, `scoring`, an optional `edge_kind` filter, `property_mode`, optional fixed `observed_at_now_unix`, `observed_at_half_life_secs`, `observed_at_weight`, and `memory_node_boost`. The default `observed_at_now_unix` is the current Unix time; tests and deterministic callers can pin it. The `PropertyMode` enum controls property inclusion: `Full` returns all properties, `Sample` keeps properties only on the first node of each kind (subsequent same-kind nodes get empty properties, edges always stripped), `Omit` strips all properties from both nodes and edges. Default is `Full`. `Sample` targets kind-redundant property maps where many nodes share the same keys. The result includes `pruned_count` and `total_discovered` diagnostics so operators can observe how many nodes were dropped by the budget. The default store path scores and sorts fetched sibling candidates before applying the `max_nodes` cutoff; SQLite mirrors the same scoring expression in SQL. When pruning occurs (`pruned_count > 0`), the `ranked_neighborhood_comparison` includes a `community_truncation_summary` that runs community detection on the kept-edge subgraph and reports how many communities were fully kept, partially pruned, or fully pruned, along with the node kinds and top labels of fully-pruned communities. This achieves an estimated 30–60% token reduction for `explain` and `neighborhood` previews by avoiding the full unranked traversal + post-hoc scoring path.

**Semantic-seeded neighborhood** (`GraphStore::semantic_seeded_neighborhood`) is
the GraphStore-level contract used by `graph-db related` after semantic seed
selection. `SemanticSeededNeighborhoodOptions` carries `depth`, output `limit`,
per-node `edge_scan_cap`, and `node_discovery_cap`. The default store ranks
incident/outgoing expansion edges before caps, batches candidate node lookup per
ranked expansion through `nodes_by_ids`, and returns skip/truncation counters for
CLI diagnostics. SQLite overrides the expansion hook with a SQL-side indexed
union over incoming/outgoing edges, ranks by the same edge-kind/direction score,
applies the edge cap in SQL, and uses chunked `IN` node lookup for discovered
candidates.

Traversal graph projection materializes indexed AST spans as stable `ast_span` graph nodes using the same handles exposed by `symbol-read` / `markdown-ast`. The projection links files, symbols, calls, routes, imports, Markdown sections, and AST spans with `has_ast_span`, `contains`, `parent`, `child`, `previous_sibling`, `next_sibling`, `enclosing_module`, `enclosing_section`, `contains_markdown_block`, `calls`, `handled_by`, `handles_route`, and `imports` edges so graph-db neighborhoods can navigate code structure without rereading source files. Markdown fenced code blocks with supported languages are parsed as embedded language islands; their extracted symbols are projected as `ast_span` nodes with `embedded=true` and linked back to the fence and enclosing section through `contains_embedded_symbol`, `embedded_in_fence`, and `contains_embedded_code` edges. Symbol and route projection coordinates are normalized from the index's zero-based storage convention to the one-based `line` contract used by `search`, `explain`, and source envelopes.

Compaction policy reports `large_projection` rather than `not_needed` when a projection is at least 256 MiB and averages at least 1024 bytes per live node or edge while ordinary tombstone/freelist compaction is not indicated. Its proof includes bytes per live row so operators can distinguish projection breadth or property density from reclaimable SQLite space.
...
**Cache eviction.** `cycle_packet_cache_evict` scans `.tsift/cycle-packet-cache/` and removes entries older than a configurable TTL (default 24 hours) based on file modification time. After TTL-based eviction, if total cache size exceeds a configurable max (default 50 MB), oldest entries are removed until the budget is satisfied. `apply_status_fixes` runs eviction during `tsift status` auto-fix (the default; `--no-fix` skips it), reporting evicted count and bytes to stderr. Eviction is part of the `.tsift/`-only fix class, so it still runs for a bare `status`; tracked instruction rewrites moved out of that function into `apply_status_instruction_fixes`. `cycle_packet_cache_stats` returns entry count and total bytes for reporting.
...
**Local mode.** `LibsqlGraphStore::open(path)` opens a local libsql database file using the same schema tables and indexes as `SqliteGraphStore` (`graph_nodes`, `graph_edges`, `graph_node_properties`, `graph_edge_properties`, `graph_node_semantic_vectors`, `graph_projection_versions`, `graph_tombstones`). The schema version and foreign key constraints are identical to the SQLite store.
...
| `GraphStore::ranked_neighborhood` (`packages/tsift-core`) | A per-call ranked layer-slot chain seeded by center node, scoring options, queue state, seen set, and fetched expansion. | One ranked-neighborhood request; there is no cross-call reuse. | Queue exhaustion, depth, max-node, edge-kind, or scoring-option changes are call-local and rebuild the Context. | `ranked_neighborhood_breadth_first_respects_max_nodes`; `ranked_neighborhood_edge_kind_weighted_prefers_high_score_edges`; `ranked_neighborhood_degree_weighted_prefers_low_degree`; `ranked_neighborhood_prefers_recent_memory_nodes_when_pruning`; `sqlite_ranked_neighborhood_prefers_recent_memory_nodes_when_pruning`. |
| `GraphStore::semantic_seeded_neighborhood` (`packages/tsift-core`) | A per-call semantic seeded queue with ranked expansion-edge caps, batched candidate node lookup, discovery cap counters, and seed-order output sorting. | One semantic seeded retrieval request; there is no cross-call reuse. | Seed set, depth, output limit, edge-scan cap, node-discovery cap, or graph mutations between calls rebuild the traversal. | `semantic_seeded_neighborhood_scores_before_caps`; `sqlite_semantic_seeded_neighborhood_scores_before_sql_edge_cap`; `graph_db_semantic_seeded_neighborhood_scores_before_caps`. |
