...
- Exact and lexical search hit previews add a nested `source_handle` window when the hit has a line anchor, plus `containing_symbol` metadata when an indexed symbol span encloses that line. These additions do not replace the existing `shit-*` retrieval handle or path-narrowing `expand` command; they give agents a bounded `source-read` window before resorting to broad file reads. The ranked path applies a test-path penalty and a definition/source-path boost so production definitions and high-signal symbols appear before test-only matches when scores are otherwise close.
...
- `session-cost --compact` and `session-review --compact` print one-line `prompt-cache-roi` rows when prompt-cache economics are present. Each row carries provider/session context, net cached-read tokens, read/create ratio, trend, suspected invalidation cause, and the one next `session-cost` command needed to refresh the measurement before another long agent-doc cycle.
...
`tests/exit_code.rs` also runs `tsift session-cost --fixture fixtures/real-session-prompt-cache-effectiveness.json --fail-under --json`. That fixture is the prompt-cache economics gate: each case carries real-session-style Claude/Codex usage rows plus minimum cached-input ratio, minimum net cached-token, and maximum read/create-regression thresholds. It also includes provider/cache-key/breakpoint/routing metadata so the prompt-cache timeline proves stable-prefix fingerprints are emitted with the effectiveness report. The checked-in fixture declares required regression-scenario coverage for volatile generated prefix headers, cold standalone compaction, OpenAI `prompt_cache_key` churn, and replica routing churn; JSON/human/compact output reports required, covered, and missing scenarios, and `--fail-under` exits non-zero when required scenario coverage disappears. `session-cost` classifies provider adapter evidence in `prompt_cache_plan.provider_adapters`: Anthropic calls must prove `cache_control`, OpenAI calls must prove `prompt_cache_key`, and replica-local caches must prove stable `routing_affinity`; cases that intentionally cover key/routing churn must still prove the churn field while keeping cache reads ahead of creation work. The command reports pass/fail per case and exits non-zero under `--fail-under` when the cached prefix is not measurably effective or the source-specific adapter evidence is missing, partial, or unexpectedly churned.
