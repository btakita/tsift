...
4. Emit a bounded `prompt_cache_plan` when provider cache usage is observed or when uncached prompt volume is large enough to justify cache setup. The plan is diagnostic rather than a live model adapter: it records cache status, observed cached-read/creation tokens, stable-prefix invariants, provider adapter requirements, and next actions. It also includes bounded `analytics` for reviewing effectiveness over time: sample count, effective/stable/improving/declining trend, average/first/last cached-input ratios, net cached-read versus creation tokens, read/create ratio when creation tokens exist, likely invalidation diagnostics for cached-ratio drops, cache-creation spikes, and read/create regressions, a prompt-cache prefix drift report, and a first-plus-latest timeline of per-turn cache ratios. A compact `scorecard` groups the same economics by provider with sample count, net cached-read tokens, read/create ratio, trend, suspected invalidation cause, and the next `session-cost` command to rerun before a long agent-doc cycle. Each timeline call carries prompt-cache attribution metadata when usage is parsed: provider, optional cache key, stable-prefix fingerprint, raw stable-prefix content, explicit breakpoint paths, and optional routing affinity. Breakpoint paths are position-independent — JSON array indices are stripped (`message.content[N].cache_control` → `message.content.cache_control`) and repeated identical shapes collapse to a single counted entry (`... (xN)`) — so inserting a non-cached block ahead of the cached one does not shift the breakpoint identity and read as false cache-boundary drift. The prefix drift report compares adjacent calls by stable-prefix fingerprint, raw stable-prefix content, cache key, explicit breakpoint paths, routing affinity, and provider, then marks cache-ratio drops or creation spikes with the first changed field. Each prefix-drift entry also carries a concrete, field-specific `remediation` so the report states what to *do* about the attributed cause, not just what changed: stabilize the named input or move the volatile field below the cache breakpoint (for example a changed `cache_key` recommends reusing the same thread/session id, a changed `stable_prefix` recommends moving the volatile content below the cache breakpoint, and `routing_affinity` recommends pinning the session to one cache replica). The remediation is always populated, including an actionable fallback for any future field. Because the stable-prefix fingerprint is typically a *derived* composite of provider, cache key, stable prefix, and breakpoints, attribution orders the concrete sub-fields (cache key, breakpoints, routing affinity, provider, then raw stable prefix) ahead of the composite fingerprint, so `first_changed_field` and the scorecard's suspected invalidation cause name the specific changed input. The raw stable-prefix content is tracked as its own field — ordered immediately before the fingerprint — so that a real prefix-content drift is attributed to `stable_prefix` regardless of whether the fingerprint is derived from that material or supplied explicitly by the provider: an explicit `stable_prefix_fingerprint` (`stablePrefixFingerprint`/`prefix_fingerprint`/`prefixFingerprint`) bypasses the derived material, so without the raw-prefix field a genuine prefix-content change would alter nothing tracked and go unattributed. A *derived* fingerprint is a pure function of the tracked sub-fields (provider, cache key, raw stable prefix, breakpoints), so whenever it changes one of those sub-fields changed too and is already reported — the derived fingerprint entry is a redundant echo and is suppressed from the field-change list. The composite fingerprint is reported only when the provider supplied it *explicitly*, i.e. as independent signal not captured by any tracked sub-field (e.g. the provider rotated an explicit fingerprint independently of the prefix content). Position-independent breakpoint entries carry their cardinality with a trailing `(xN)` count; provider-supplied breakpoint text that already ends in a `(xN)`-shaped suffix is escaped to `(\xN)` so a literal can never be confused with the appended count (which keeps the breakpoint multiset injective for drift comparison). The provider notes must distinguish Anthropic explicit breakpoints (`cache_control` on stable system/tool/user boundaries), OpenAI cache-key routing (`prompt_cache_key` for byte-identical thread prefixes), and self-hosted/edge affinity needs when cache locality is replica-bound. The invariants should preserve the core prompt-caching shape from agent runtimes: stable context first, append-only thread history until intentional compaction, no volatile values inside the cached prefix, and compaction calls that reuse the live warm prefix instead of starting a cold standalone summarizer.
...
3. Discover related Claude sessions under `~/.claude/projects/<cwd-slug>/`, Codex sessions under `~/.codex/sessions/`, and `agent-doc` runtime logs under `<root>/.agent-doc/logs/`. When the target itself is a recognized Claude or Codex JSONL transcript, admit that exact file with `explicit_target` provenance before discovery; reject an unrecognized explicit JSONL target with a nonzero error instead of returning an empty successful report.
...
10. `session-review` persists a per-session prompt-cache effectiveness reading across runs so cross-run regressions become detectable instead of only the per-fixture / per-transcript gate (`session-cost --fixture`). On each invocation the CLI records one sample for the latest matched session — `cached_input_ratio`, `net_cached_input_tokens` (signed cached-read minus cache-creation tokens), and `read_create_regressions` (1 when cached/creation reads dip below the `2.0` read-to-create ratio) — appended oldest-first to `<root>/.tsift/prompt-cache-history/<source>-<hash>.jsonl`. Recording is idempotent: an unchanged session (same modified-time + token totals as the last persisted row) is not re-appended, and the history is capped at a trailing window (200 samples). The new sample is compared against the previous distinct recorded reading and the report adds `prompt_cache_cross_run` carrying the previous/current samples plus a `regressions` list with `cached_input_ratio_drop` (ratio fell at least 5 points), `net_cached_input_tokens_drop` (net halved while positive, or crossed into negative — the session now spends more on cache creation than it saves), and `read_create_regressions_increase` entries. Recording is best-effort: a read-only working tree logs a stderr note and still returns the read-only review.

11. `--next-context` surfaces prompt-cache health inline as `next_context.prompt_cache_health` so caching effectiveness and the top prefix-drift attribution show in the resumable handoff without separately running the `session-cost --fixture` gate. The health summary is built from data available at review time — the aggregate `cached_input_ratio` and the latest matched session's prompt-cache ROI scorecard row (net cached-read tokens, read/create ratio, trend, and the suspected prompt-cache invalidation cause = top drift attribution) — and carries a `status` (`healthy` | `watch` | `regressed`) plus a one-line `summary_line` for compact/human renderings. Base status is `regressed` when net cached-read tokens are negative (cache creation now outweighs reads), `watch` when a non-empty drift attribution is present, otherwise `healthy`. When the cross-run comparison from item 10 reports any regression, those regression detail lines are folded into `cross_run_regressions` and the status escalates to `regressed`. The field is omitted when no cache signal is available. `session-review --next-context`, the budgeted next-context report, and `context-pack` (compact and verbose) all print the `summary_line`.
...
When the target itself is a Claude or Codex JSONL transcript outside the source tree, root resolution uses the first bounded-header `cwd` record (Claude top-level `cwd`; Codex `session_meta.payload.cwd`) before composing index, diff, status, and graph sections. The harness transcript directory is never treated as a source root merely because it contains the target file; absent a usable embedded cwd, normal target-path root resolution remains the fallback.
