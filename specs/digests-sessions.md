...
3. Fold near-duplicate lines into semantic line families. Beyond exact-line collapsing, each line is reduced to a template by replacing its variable tokens — bracketed epoch timestamps, file paths, semantic versions, hex hashes/object ids, numbers/durations/sizes/percentages, the value side of `key=value` structured fields, and the cargo-style crate name immediately before a bare version — with stable placeholders. Lines sharing a template are grouped into a family carrying its template, total occurrence count, distinct-variant count, and first/last raw samples. Only families with more than one distinct variant (genuine near-duplicates that exact-line collapsing misses, such as cargo/build/install progress, dependency downloads, and repeated harness lifecycle lines that vary by one path, count, timestamp, or crate name) are reported, ranked by occurrence count and bounded to the top families with the full family count preserved. `line_families` is omitted from JSON when empty.
