...
Everything else refuses, and the refusal list is load-bearing: a rename that misses an occurrence breaks the build loudly, but an extraction with a wrong parameter list can compile and silently change behaviour. Refusals cover a range that is not a contiguous run of siblings in one block, a range outside any function, an enclosing function that sits in an *expression* rather than a statement (an arrow function, a function expression, a class expression — there is no statement slot beside it), control flow that escapes the range (`return` and `yield` always; `break` and `continue` only when the loop or `switch` they bind to stays behind, and a labelled branch only when its label does), a range that names `this` or `super`, a range assigning a name `F` declared `global`/`nonlocal`, a new name that already binds in `F`, at module scope, or beside the insertion point, several returned names in a language with no destructuring call site, a return set that mixes newly declared with already declared names, and a parameter whose type cannot be spelled.

### `break` and `continue` are conditional

`return` and `yield` escape unconditionally: no signature carries them. `break` and `continue` depend on what the selection contains. A range holding a whole loop takes that loop's `break` with it and nothing changes; a range holding only the loop's *body* leaves the loop behind, and hoisting the same `break` changes which construct it leaves. Refusing both cases would decline the most ordinary extraction there is — pull a loop out of a function — so the check counts the loops (and, in JavaScript, the `switch`es, which `break` also binds to) between the branch and the edge of the selection. A labelled branch ignores that nesting and jumps to its label, so it is checked against the labels the range itself carries.
