...
2. Claim the coarse `summaries.lock` sidecar so only one extractor mutates a cache at a time, then classify files by content hash and remove stale rows. Zero-byte source files are not extraction candidates and never reach the model. Fully cached runs require no model credentials. A cached `too_large` failure stops applying automatically when the effective `--max-file-tokens` / `.tsift/config.toml` cap rises above the cap that produced it.
