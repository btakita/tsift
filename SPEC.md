...
The graph orchestration JSON contract versions are intentionally explicit: `graph-db-evidence-v1`, `worker-prompt-packet-v1`, `conflict-matrix-v1`, `context-pack-graph-orchestration-v1`, `session-review-follow-up-v1`, and `dispatch-trace-v1`. `tsift graph-db --path . schema --json` lists those versions, `fixtures/graph-db-operator-examples/graph-orchestration-contracts.json` contains compact example envelopes for agent-doc consumers, and `fixtures/graph-db-operator-examples/agent-orchestration-acceptance-pack.json` contains a queue-run acceptance pack with job packets, worker results, conflict-matrix output expectations, dispatch-trace replay expectations, and context-pack/session-review follow-up commands. The compiled graph conformance suite regenerates the acceptance pack's normalized `graph-db refresh`, `graph-db evidence`, `conflict-matrix`, `dispatch-trace` JSON/HTML, `context-pack`, and `session-review` samples from the real queue fixture and fails CI when the checked-in `regenerated_samples` contract drifts. Evidence packets persist `packet_id` and `projection_hash`; replay fixtures should rerun the emitted `replay_commands` against the same target and fail closed through the emitted `repair_commands` whenever freshness reports `missing` or `stale`.
