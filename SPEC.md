...
SQLite neighborhood paging builds the reachable node page and page-scoped edges from one recursive reachable-set CTE so node paging and edge materialization do not repeat the bounded walk. The conformance suite locks this shared-CTE diagnostic alongside SQLite and Convex snapshot parity.
