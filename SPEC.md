...
**Pushdown queries.** `LibsqlGraphStore` overrides `edge()` and `incident_edges()` with indexed SQL queries instead of relying on the default `all_edges()` scan. `edge()` probes `graph_edges` by `edge_key`. `incident_edges()` uses a UNION of two index probes on `from_id` (via `idx_graph_edges_from_kind`) and `to_id` (via `idx_graph_edges_to_kind`), matching the SQLite backend's `sqlite_incident_edges_union_query` pattern. Both overrides avoid the O(E) full-table scan that the default `GraphStore` implementations would incur.
