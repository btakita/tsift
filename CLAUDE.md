...
| `tsift rewrite` | Shell command → tsift equivalent (for Claude Code hook integration), including exact-search rewrites plus digest-routing for `git diff`, `cargo test` / `pytest`, and verbose cargo build/check/install flows |
...
| `tsift diff-digest` | Code-aware digest for the current git diff. Compares `HEAD` to the working tree (plus untracked files), reports changed files, touched symbols, current cached summary snippets when `summaries.db` matches the file content, and added/removed call edges without requiring a fresh `index.db`. |
...
| `tsift session-digest` | Bounded session transcript digest for markdown session docs and Claude JSONL. Extracts prompt targets, shell commands, touched files/symbols, failures, and closeout evidence without replaying the transcript. |
...
- **Search rewrite** (`PreToolUse`): `~/.claude/hooks/tsift-rewrite.sh` rewrites `rg`/`grep -r` to `tsift search --exact`, plain `git diff` to `tsift diff-digest`, `cargo test` / `pytest` to the tsift test-digest wrapper, and verbose cargo build/check/clippy/install commands to the tsift log-digest wrapper.
