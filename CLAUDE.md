...
| `tsift rewrite` | Shell command → tsift equivalent (for Claude Code hook integration), including exact-search rewrites plus digest-routing for `git diff`, `git diff --cached`, `git show`, simple patch-style `git log -p -1 ...`, long session transcript reads (`cat` / `head` / `tail` / `sed -n` over agent-doc markdown or Claude JSONL), `cargo test` / `pytest`, and verbose cargo build/check/install flows |
