...
- **Semantic edit intents fail closed**: `edit-intents` validates AST edit intent packets, resolves file/symbol targets with stable span handles when indexed spans are available, detects content-hash conflicts, and supports `--apply` for Rust `rename_symbol`, `replace_function_body`, `insert_import`, `add_method`, `rewrite_call_sites`, `update_call_signature`, and `move_declaration` intents. Call-site rewrites use indexed same-file refs; `move_declaration` currently requires an existing same-directory destination file and normalizes the source module with `mod`/`use`; cross-file call refs, missing `call_replacement` for signature updates, unsupported structural destinations, unsupported kinds/languages, and formatter failures do not mutate files.
