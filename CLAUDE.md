...
| `tsift edit-intents` | Semantic write-intent validator/executor: accepts JSON intent batches (`rename_symbol`, `replace_function_body`, `insert_import`, `add_method`, `update_call_signature`, `move_declaration`, `rewrite_call_sites`), emits dry-run target ranges/spans/indexed call refs/structural destinations/content hashes/diff previews, verifies supported intents in a temp git worktree with `--verify`, and applies supported Rust/TS/JS/Python intents with `--apply`. |
