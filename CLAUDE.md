...
- **Read-only summary lookups**: `tsift summarize --stats`, `tsift summarize <symbol>`, `tsift summarize --file <path>`, and `tsift diff-digest` open `summaries.db` read-only, fail closed when the cache is absent, and retry through a snapshot copy when a rollback-journal lock wedges the live cache, so query-mode summary/digest commands do not create or contend on the summary DB.
