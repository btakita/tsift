...
| `tsift symbol-read` | Symbol-centered read packet: resolves an indexed symbol, emits AST span metadata, a token-budgeted body preview, child symbol refs, cached summary refs, and expansion commands for source windows, `explain`, callers, and callees. Markdown spans include heading section ranges, section paths, list depth, and code-fence language metadata. |
| `tsift edit-intents` | Semantic write-intent validator/executor: accepts code intent batches (`rename_symbol`, `replace_function_body`, `insert_import`, `add_method`, `update_call_signature`, `move_declaration`, `rewrite_call_sites`) plus Markdown intents (`rename_heading`, `replace_section_body`, `insert_section`, `move_section`, `insert_list_item`, `rewrite_code_fence`), emits dry-run target ranges/spans/indexed call refs/structural destinations/content hashes/diff previews, verifies supported code and Markdown intents in a temp git worktree with source-read/impact evidence via `--verify`, and applies supported Rust/TS/JS/Python intents plus Markdown section/block intents with `--apply`. |
...
| `tsift traverse` | Graphify-style graph reports over files, symbols, routes, agent-doc sessions, backlog items, queue job packets, source handles, and worker context. Materializes `.tsift/graph.db` through the provider-neutral `GraphStore` before reading reports. `--convex-snapshot <rows.json>` validates Convex freshness and fails closed when the projection trails SQLite. |
| `tsift convex-sync` | Dry-run or live Convex `nodes` + `edges` sync plan for the local graph projection. Emits idempotent upserts, tombstones, chunks, required indexes, projection freshness diagnostics, optional `--snapshot <rows.json>` parity checks, and `--apply` transport compatibility with `examples/convex-graph`. |
...
| `tsift status` | Session health check: index freshness, instruction version, summary cache, recommended commands. Workspace roots treat scoped indexes under `.tsift/indexes/<scope>/index.db` as the authoritative status surface, even if a shared `.tsift/index.db` also exists; they surface configured-but-missing scopes instead of reporting false `fresh`, recommend `--workspace` rebuilds, and include summary-cache recovery diagnostics when status had to fall back to a snapshot. `--fix` applies safe local index/instruction refreshes before reporting. `--json` auto-fixes stale indexes/instructions and includes per-scope indexed status plus `missing_scopes`. |
...
- **Traversal graph uses GraphStore**: `traverse` and `context-pack` materialize provider-neutral graph rows into `.tsift/graph.db`; Convex snapshot validation fails closed when projection metadata or rows are stale relative to SQLite. `graph-db kind` and `graph-db neighborhood` support node-id cursor pagination plus repeatable `--property KEY=VALUE` filters through materialized SQLite property rows across SQLite and Convex snapshots, `graph-db compact` reports guarded storage-compaction proof, and `graph-db doctor` validates local/Convex projection health without refreshing first.
- **Convex graph backend example**: `examples/convex-graph` is the app-side schema/mutation/HTTP-action package for `tsift convex-sync --remote-snapshot --apply`; `fixtures/graph-db-operator-examples` contains operator commands and a stale snapshot fixture.
...
- **Semantic edit intents fail closed**: `edit-intents` validates AST edit intent packets, resolves file/symbol targets with stable span handles when indexed spans are available, detects content-hash conflicts, and supports `--verify` for detached temp-worktree apply/reindex/source-read/impact checks plus optional `--verify-command` gating before mutation for code and Markdown intents. `--apply` supports Rust `rename_symbol`, `replace_function_body`, `insert_import`, `add_method`, `rewrite_call_sites`, `update_call_signature`, and `move_declaration` intents plus TypeScript/TSX, JavaScript/JSX, and Python `rename_symbol`, `replace_function_body`, and `insert_import` intents. Markdown `.md`/`.mdx` files resolve to canonical language id `markdown`; Markdown headings now index full section spans with hierarchy metadata, list items expose list depth and section path, and fenced code blocks expose code-body ranges plus fence-language metadata. Markdown section intents `rename_heading`, `replace_section_body`, `insert_section`, and `move_section` apply through tree-sitter-validated current-buffer section spans and the atomic edit path; block intents `insert_list_item` and `rewrite_code_fence` apply through unique list/code-fence targets while preserving list marker/indent and fence syntax. Rust output requires `rustfmt`; script-language output is tree-sitter validated and uses local `prettier`, `ruff format`, or `black --quiet` when available. Call-site rewrites use indexed same-file refs; `move_declaration` currently requires an existing same-directory destination file and normalizes the source module with `mod`/`use`; ambiguous Markdown block targets, cross-file call refs, missing call replacements, unsupported structural destinations, unsupported kinds/languages, AST validation mismatches, verification failures, and formatter failures do not mutate files.
...
- **Cross-harness fallback**: when your harness does not offer Claude-style `PreToolUse` hooks, run `tsift rewrite --run '<command>'` for broad search, raw session/transcript/log reads, git diff/show/log patch review, cargo/pytest tests, and cargo build/check/clippy/install output. It executes the same envelope-first rewrite directly, including search preview budgets and digest-runner artifact envelopes. OpenCode can install the `/tsift-rewrite-run` shortcut with `tsift init --opencode`. When RTK is installed and supports the wrapped command, digest-runner delegates the command output filter to RTK and still wraps the result in tsift metadata.
...
Run `tsift status` at session start from the owning repo root. If the task or file lives under a git submodule (for example `src/tsift/...`), switch to that submodule root first so the harness loads the narrower local instructions and repo state instead of the superproject root. If status prints a `run:` recommendation for stale or missing tsift state, `tsift status` auto-fixes by default; use `--no-fix` to skip auto-fixing. When the harness cannot perform write commands, ask the user to run the printed command instead. Codex projects can install a prompt-time auto-reindex hook with `tsift init --codex`; OpenCode projects can install per-project tsift command shortcuts with `tsift init --opencode`.
