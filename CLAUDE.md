...
| `tsift traverse` | Graphify-style graph reports over files, symbols, routes, agent-doc sessions, backlog items, and source handles. Materializes `.tsift/graph.db` through the provider-neutral `GraphStore` before reading reports. `--convex-snapshot <rows.json>` validates Convex freshness and fails closed when the projection trails SQLite. |
| `tsift convex-sync` | Dry-run Convex `nodes` + `edges` sync plan for the local graph projection. Emits idempotent upserts, tombstones, chunks, required indexes, projection freshness diagnostics, and optional `--snapshot <rows.json>` parity checks. |
...
- **Traversal graph uses GraphStore**: `traverse` and `context-pack` materialize provider-neutral graph rows into `.tsift/graph.db`; Convex snapshot validation fails closed when projection metadata or rows are stale relative to SQLite.
