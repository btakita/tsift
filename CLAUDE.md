...
| `tsift edit-intents` | Semantic write-intent validator: accepts JSON intent batches (`rename_symbol`, `replace_function_body`, `insert_import`, `add_method`, `update_call_signature`, `move_declaration`, `rewrite_call_sites`) and emits dry-run target ranges/content hashes without mutating files. |
...
- **Semantic edit intents are dry-run**: `edit-intents` validates AST edit intent packets, resolves file/symbol targets, and detects content-hash conflicts, but leaves mutation disabled until per-language executors prove formatting and rollback behavior.
