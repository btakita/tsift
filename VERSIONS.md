...
- **Fix the CI-only flake in the search post-precheck lock tests (`#ciwalflake`).** `search_cmd_uses_wal_snapshot_fallback_when_lock_appears_after_precheck` reddened CI on the v0.1.100 release commit (479 passed / 1 failed) while the full local suite stayed green. The test harness waited at most **1 second** for a spawned thread to open SQLite, switch journal modes, create a probe table, insert, take an exclusive lock, and confirm the WAL sidecar — a latency assertion about a contended runner, not a readiness wait. The WAL hook does strictly more setup than its rollback-journal sibling, which is why it was the one that blew the budget. The bound is now 30s and is a deadlock-breaker rather than a timing window. The harness also held the lock for a fixed `sleep(200ms)`, so a slow runner could release it *before* the search reached the contended read — passing the assertion while exercising none of the fallback it claims to test; the holder now waits on a release channel that the test's guard disconnects when it drops, bounded by the same 30s. Finally, both fallback tests asserted a bare `result.is_ok()`, so the red CI run reported only `assertion failed: result.is_ok()` with no cause; they now carry the error into the failure message. Verified by reproducing the original failure (1s bound + 1.5s injected setup latency → fails, now naming `waiting for search post-precheck lock hook`) and confirming the 30s bound passes under the same latency, with no change to normal-case runtime.

- **`tsift audit` now checks the version a skill declares (`#skillversioneval`).** A tsift skill pinned to an older release passed every existing check — it has a `SKILL.md`, it has a `description` — so the audit reported a 38-versions-stale skill (`0.1.62` against a `0.1.100` binary) as *healthy* while the agent loaded its command text. Nothing about a stale file's appearance reveals the drift, so the scan now reads the version the skill declares, from the `<!-- tsift:skill v=… -->` ownership marker or a `tsift-version:` frontmatter field (marker wins), and reports a mismatch as an issue that moves the skill from `healthy` to `broken`. The declared value surfaces as `declared_tsift_version` in JSON. A skill declaring neither is not tsift's and is never flagged. The expected version is the running binary's own `CARGO_PKG_VERSION`, not a separately maintained constant that could drift on its own.
- **The generated skill absorbs what the hand-maintained Claude skill carried (`#claudeskillmerge`).** Operators kept a private, richer `~/.claude/skills/tsift/SKILL.md` because the generated one was thinner — and that private copy is what went stale. The generated skill now carries `user-invocable: true` + `argument-hint` (so `/tsift` works as a slash command), `TRIGGER:`/`SKIP:`/`VERSION CHECK:` guidance in its description, a `tsift-version` frontmatter stamp, a version-check paragraph, a compact command surface, and a note that harnesses without a `PreToolUse` rewrite must drive the envelope commands themselves. It stays the hot-path router: the full command surface and version-drift detail live in `references/code-navigation.md`, and template gates enforce that the skill remains smaller than its reference and teaches no deprecated flag.
